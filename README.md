...
| 📱 marble-tilt | 기울임 마블 | ~35초 | **활성** — 폰을 좌우로 기울여 자기 마블에 실시간으로 힘을 가하는 자이로 조작 |
| ⚡ reaction | 동시탭 반응속도 | ~8초 | **활성** — 회색 "준비…" → 노란 "지금!" 동시 탭, 가장 빠른 사람이 1등 |
| 🧠 trivia | 일반 상식 퀴즈 | ~30초 | **활성** — 4지선다 5문제, 속도·콤보 보너스, 마지막 문제 점수 2배 |
| 🤪 nonsense | 넌센스 퀴즈 | ~30초 | **활성** — trivia 엔진 재사용, 말장난·언어유희 300문제 풀 |
| 🎰 slot | 슬롯머신 룰렛 | ~8초 | 비활성 |
...
- **남용 가드**: 전역 동시 방 수 상한(`ROOM.MAX_ROOMS`)으로 무한 방 생성 차단(단일 인스턴스 OOM 방지) + 방 생성 IP 레이트리밋(프로덕션, 초과 시 429)
...
- [`src/games/marble-tilt/`](src/games/marble-tilt/CLAUDE.md) — 자이로 기울임 실시간 변형 (트랙·물리 공유)
...
- [`src/games/nonsense/`](src/games/nonsense/CLAUDE.md) — 넌센스 퀴즈, trivia 엔진 재사용·문제 풀만 소유
