...
- **막힘 슬롯 금지**: 벽과 장애물 사이 틈이 `0 < 틈 < 마블 지름`이면 테이퍼 쐐기가 돼 마블이 영구 고착된다(anti-stuck 쉐이크로도 탈출 불가). 응원 마블은 충전 시 지름 0.5→0.41m로 줄어드는 것까지 감안 — 막을 거면 벽에 겹쳐 심어 틈 0, 열 거면 최근접 거리 ≥0.7m. 검증은 완주 테스트를 charged=1.0으로도 돌려볼 것.
