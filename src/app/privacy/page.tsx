...
export const metadata: Metadata = {
  title: `${ko.privacy.title} · ${ko.app.title}`,
};
  ...
  return (
    <main className="min-h-dvh flex flex-col px-6">
      <article className="flex-1 mx-auto w-full max-w-lg py-10">
        ...
        <h1 className="mt-4 text-2xl font-bold">{ko.privacy.title}</h1>
        <p className="mt-1 text-xs text-zinc-500">{ko.legal.updated(ko.privacy.updatedAt)}</p>
        <div className="mt-7 space-y-6">
          {ko.privacy.sections.map((s) => (
            <section key={s.h}>
